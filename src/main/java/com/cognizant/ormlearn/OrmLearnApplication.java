...
public class OrmLearnApplication {
	...
	CommandLineRunner testAddEmployee() {
		return args -> {
			...
			Employee.builder().name("Maneesh").salary(400000.00).permanent(true)
					.dateOfBirth(new SimpleDateFormat("yyyy-MM-dd").parse("1999-09-01")).build();
			Department department = departmentService.findDepartment(3);
			Employee employee = Employee.builder().name("Upender").salary(700000.00).permanent(false)
					.dateOfBirth(new SimpleDateFormat("yyyy-MM-dd").parse("1998-07-02")).department(department).build();
